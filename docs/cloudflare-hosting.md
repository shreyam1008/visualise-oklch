...
- Production builds watch `docs/*` and `scripts/prepare-pages.mjs` (the legacy `screenshot.png` has been removed). Images now live under `images/`, copied by the build; edit docs when refreshing them to trigger the configured build.
