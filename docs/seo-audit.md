...
## Custom picker showcase

The install section now leads with a sandboxed, lazy-loaded interactive demo generated from `src/pickerHtml.ts` and `src/pickerClient.ts`. Only the VS Code host bridge is replaced; edits are in memory and never touch editor files. This is labeled as a browser demo, not a current VS Code screenshot. The older inline-swatch image remains supplementary. The demo is regenerated by `build:site`, uses no remote dependencies, and is noindex to keep the main page canonical for discovery.
